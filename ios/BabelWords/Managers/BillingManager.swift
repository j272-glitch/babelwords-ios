...
final class BillingManager: NSObject {
    ...
    private func handleVerification(_ verification: VerificationResult<Transaction>, productId: String) async {
        ...
        case .verified(let transaction):
            ...
            await validateWithServer(productId: productId, transactionId: transaction.id)
    ...
    private func validateWithServer(productId: String, transactionId: UInt64) async {
        guard let url = URL(string: "https://linguagt.com/api/iap/apple/verify") else { return }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        let body = ["productId": productId, "transactionId": String(transactionId)]
        do {
