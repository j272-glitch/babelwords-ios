...
[
  {
    ...
    "target": {
      ...
      "package_name": "com.linguawonder.app",
...
Confirm `package_name` is correct (currently `com.linguawonder.app`) and replace the
...
5. On a device with the new build installed:
   ```bash
   adb shell pm get-app-links com.linguawonder.app
