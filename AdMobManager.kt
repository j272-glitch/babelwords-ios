...
class AdMobManager private constructor(private val context: Context) {
    ...
    private fun requestConsent(activity: Activity, onComplete: (Boolean) -> Unit) {
        ...
        consentInformation?.requestConsentInfoUpdate(
            ...
            { formError ->
                ...
                TestRigorLogger.logError("Consent request failed: ${formError.message}", formError)
                onComplete(false)
            }
        )
    }
    ...
    private fun loadConsentForm(activity: Activity, onComplete: (Boolean) -> Unit) {
        UserMessagingPlatform.loadConsentForm(
            ...
            { consentForm ->
                if (consentInformation?.consentStatus == ConsentInformation.ConsentStatus.REQUIRED) {
                    consentForm.show(activity) { formError ->
                        if (formError != null) {
                            TestRigorLogger.logError("Consent form error: ${formError.message}", formError)
                        }
            ...
            { formError ->
                TestRigorLogger.logError("Consent form load failed: ${formError.message}", formError)
                onComplete(false)
            }
        )
    }
