 ...
 * 
 * IMPORTANT: This bridge now uses placement IDs passed from JavaScript!
 * The web app sends production AdMob IDs like "ca-app-pub-9991891515643313/5076005693"
 * 
...
class AdWaterfallBridge(
    ...
    companion object {
        ...
        // InMobi Configuration (fallback if JS doesn't provide)
        private const val INMOBI_ACCOUNT_ID = "9d81516c365f4acaa52f1fc627370cf9"
        private const val INMOBI_BANNER_PLACEMENT_DEFAULT = 10000582111L
        private const val INMOBI_INTERSTITIAL_PLACEMENT_DEFAULT = 10000582110L
        private const val INMOBI_REWARDED_PLACEMENT_DEFAULT = 10000582112L
        
        // AdMob Configuration (fallback if JS doesn't provide)
        private const val ADMOB_BANNER_ID_DEFAULT = "ca-app-pub-9991891515643313/6878126239"
        private const val ADMOB_INTERSTITIAL_ID_DEFAULT = "ca-app-pub-9991891515643313/5076005693"
        private const val ADMOB_REWARDED_ID_DEFAULT = "ca-app-pub-9991891515643313/6313049833"
    }
    
    // Current placement IDs (set from JavaScript or use defaults)
    private var currentAdMobBannerId = ADMOB_BANNER_ID_DEFAULT
    private var currentAdMobInterstitialId = ADMOB_INTERSTITIAL_ID_DEFAULT
    private var currentAdMobRewardedId = ADMOB_REWARDED_ID_DEFAULT
    private var currentInMobiBannerPlacement = INMOBI_BANNER_PLACEMENT_DEFAULT
    private var currentInMobiInterstitialPlacement = INMOBI_INTERSTITIAL_PLACEMENT_DEFAULT
    private var currentInMobiRewardedPlacement = INMOBI_REWARDED_PLACEMENT_DEFAULT
    ...
    /**
     * Helper to determine if a placementId from JS looks like an AdMob ID
     * AdMob IDs follow pattern: ca-app-pub-XXXX/YYYY
     */
    ...
    /**
     * Helper to parse InMobi placement ID from string to Long
     */
    private fun parseInMobiPlacement(placementId: String, default: Long): Long {
        return try {
            placementId.toLong()
        } catch (e: Exception) {
            logDiag("  Could not parse InMobi placement '$placementId', using default: $default", "W")
            default
        }
    }
    ...
    fun loadBanner(placementId: String, position: String) {
        ...
        // Update current placement ID from JS if it looks like an AdMob ID
        if (isAdMobId(placementId)) {
            currentAdMobBannerId = placementId
            logDiag("  ✓ Using AdMob Banner ID from JS: $currentAdMobBannerId")
        } else {
            logDiag("  Using default AdMob Banner ID: $currentAdMobBannerId")
        }
    ...
    fun loadInterstitial(placementId: String) {
        ...
        if (isAdMobId(placementId)) {
            ...
            logDiag("  ✓ Using AdMob Interstitial ID from JS: $currentAdMobInterstitialId")
        } else {
            logDiag("  Using default AdMob Interstitial ID: $currentAdMobInterstitialId")
        }
    ...
    fun loadRewarded(placementId: String) {
        ...
        if (isAdMobId(placementId)) {
            ...
            logDiag("  ✓ Using AdMob Rewarded ID from JS: $currentAdMobRewardedId")
        } else {
            logDiag("  Using default AdMob Rewarded ID: $currentAdMobRewardedId")
        }
    ...
    private fun loadBannerWaterfall(position: String) {
        ...
        logDiag("  InMobi Placement ID: $currentInMobiBannerPlacement")
        logDiag("  AdMob Fallback ID: $currentAdMobBannerId")
        
        try {
            inMobiBanner = InMobiBanner(activity, currentInMobiBannerPlacement).apply {
    ...
    private fun loadAdMobBanner(position: String) {
        ...
        logDiag("  Ad Unit ID: $currentAdMobBannerId (from JS)")
    ...
    private fun loadInterstitialWaterfall() {
        ...
        logDiag("  InMobi Placement ID: $currentInMobiInterstitialPlacement")
        logDiag("  AdMob Fallback ID: $currentAdMobInterstitialId")
        
        try {
            inMobiInterstitial = InMobiInterstitial(activity, currentInMobiInterstitialPlacement,
    ...
    private fun loadAdMobInterstitial() {
        ...
        logDiag("  Ad Unit ID: $currentAdMobInterstitialId (from JS)")
    ...
    private fun loadRewardedWaterfall() {
        ...
        logDiag("  Ad Unit ID: $currentAdMobRewardedId (from JS)")
