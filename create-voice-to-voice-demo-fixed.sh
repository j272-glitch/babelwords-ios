...
ffmpeg -y -f lavfi -i "color=c=${PURPLE_COLOR}:size=${WIDTH}x${HEIGHT}:duration=10:rate=${FPS}" \
    -vf "drawtext=text='LinguaLink':fontsize=80:fontcolor=white:x=(w-text_w)/2:y=100:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
         ...
         drawtext=text='MIC':fontsize=60:fontcolor=${PURPLE_COLOR}:x=(w-text_w)/2:y=380:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
...
ffmpeg -y -f lavfi -i "color=c=${PURPLE_COLOR}:size=${WIDTH}x${HEIGHT}:duration=10:rate=${FPS}" \
    -vf "drawtext=text='Voice Translation in Progress':fontsize=50:fontcolor=white:x=(w-text_w)/2:y=120:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
         ...
         drawtext=text='Speech Recognition to Text':fontsize=25:fontcolor=white:x=(w-text_w)/2:y=500:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf,
         ...
         drawtext=text='Text to Speech Synthesis':fontsize=25:fontcolor=white:x=(w-text_w)/2:y=580:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf,
...
ffmpeg -y -f lavfi -i "color=c=${PURPLE_COLOR}:size=${WIDTH}x${HEIGHT}:duration=10:rate=${FPS}" \
    -vf "drawtext=text='Translation Complete':fontsize=60:fontcolor=white:x=(w-text_w)/2:y=100:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
         ...
         drawtext=text='Hola como estas hoy':fontsize=28:fontcolor=black:x=(w-text_w)/2:y=250:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf,
...
ffmpeg -y -f concat -safe 0 -i temp/filelist.txt -c copy lingualink-voice-to-voice-demo.mp4
...
echo "📁 Output: lingualink-voice-to-voice-demo.mp4"
...
if [ -f "lingualink-voice-to-voice-demo.mp4" ]; then
    echo ""
    echo "📊 Video Information:"
    ls -lh lingualink-voice-to-voice-demo.mp4
    ffprobe -v quiet -show_entries format=duration:stream=width,height,codec_name lingualink-voice-to-voice-demo.mp4 2>/dev/null || echo "Video created successfully"
else
