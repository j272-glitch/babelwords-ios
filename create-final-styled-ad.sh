...
echo "🎬 Creating Final Styled LinguaLink Ad..."
...
echo "📊 Updating third slide styling:"
echo "   🔄 'Real time device to device...' → WHITE + LARGER font"
echo "   🔒 'End-to-End Encryption' → GOLD color"
echo "   📢 'App supported by ads' → LARGER font"
echo ""
...
# Updated combined slide with new styling (10 seconds)
echo "🔒🖼️ Creating updated styled combined slide (10s)..."
ffmpeg -y -f lavfi -i "color=c=${PURPLE_COLOR}:size=${WIDTH}x${HEIGHT}:duration=10:rate=${FPS}" \
    -vf "drawtext=text='LinguaLink':fontsize=90:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2-250:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
         drawtext=text='Real time device to device conversation':fontsize=45:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2-150:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
         drawtext=text='using translated speech':fontsize=45:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2-100:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
         
         drawtext=text='End-to-End Encryption':fontsize=35:fontcolor=0xFFD700:x=(w-text_w)/2:y=(h-text_h)/2-20:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
         ...
         drawtext=text='App supported by ads':fontsize=28:fontcolor=0xC0C0C0:x=(w-text_w)/2:y=(h-text_h)/2+240:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf,
         fade=in:st=0:d=0.5,fade=out:st=9.5:d=0.5" \
...
echo ""
echo "✅ Final Styled LinguaLink ad complete!"
echo "📁 Output: output/lingualink-ad-enhanced.mp4"
echo "🎯 Updated Third Slide Styling:"
echo "   📱 'Real time device to device conversation using translated speech'"
echo "       🎨 Color: WHITE (was gold)"
echo "       📏 Font: 45px (increased from 38px)"
echo "   🔒 'End-to-End Encryption'"
echo "       🎨 Color: GOLD (was white)"
echo "   📢 'App supported by ads'"
echo "       📏 Font: 28px (increased from 20px)"
...
if [ -f "output/lingualink-ad-enhanced.mp4" ]; then
    ...
    echo "❌ Error: Final styled ad was not created"
fi