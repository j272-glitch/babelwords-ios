...
echo "🎬 Creating Simple Corrected LinguaLink Ad..."
...
# Create CTA section (20 seconds) with simplified contact text
...
ffmpeg -y -f lavfi -i "color=c=0x4B3FD8:size=${WIDTH}x${HEIGHT}:duration=20:rate=${FPS}" \
    -vf "drawtext=text='Ready to Connect Globally?':fontsize=80:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2-200:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
         ...
         drawtext=text='Contact gtlingua at pm.me':fontsize=35:fontcolor=0xE0E0E0:x=(w-text_w)/2:y=(h-text_h)/2+180:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf,
         drawtext=text='Visit gtlingua.com':fontsize=35:fontcolor=0xE0E0E0:x=(w-text_w)/2:y=(h-text_h)/2+230:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf,
         fade=in:st=0:d=1" \
