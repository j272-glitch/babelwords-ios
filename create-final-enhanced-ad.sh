...
echo "🎬 Creating Final Enhanced LinguaLink Ad..."
...
echo "📊 Creating 4 slides of exactly 7.5 seconds each:"
echo "   🔄 Changed to 'device-to-device conversation'"
echo "   ⏱️ Each slide: 7.5 seconds (total: 30s)"
echo ""

# Create intro (7.5 seconds)
...
# Create D2D conversation slide (7.5 seconds)
...
# Create finale with device-to-device conversation highlight (7.5 seconds)
echo "🖼️ Creating finale with D2D conversation highlight (7.5s)..."
ffmpeg -y -f lavfi -i "color=c=${PURPLE_COLOR}:size=${WIDTH}x${HEIGHT}:duration=7.5:rate=${FPS}" \
    -vf "drawtext=text='LinguaLink':fontsize=100:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2-180:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
         drawtext=text='Device-to-Device Conversation':fontsize=45:fontcolor=0xFFD700:x=(w-text_w)/2:y=(h-text_h)/2-80:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
         drawtext=text='Break Language Barriers':fontsize=40:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2-30:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf,
         drawtext=text='● → ♪':fontsize=80:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2+50:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf,
...
echo ""
echo "✅ Final Enhanced LinguaLink ad complete!"
echo "📁 Output: output/lingualink-ad-enhanced.mp4"
echo "🎯 Perfect Structure:"
...
echo "   🖼️ Finale (7.5s) - Device-to-device conversation highlight"
echo "⏱️ Total: Exactly 30 seconds"
echo ""
echo "🆕 Updated Features:"
echo "   ✅ 'Device-to-Device Conversation' (not translation)"
echo "   ✅ Consistent 7.5 second slides"
