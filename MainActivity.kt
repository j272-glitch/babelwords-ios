...
class MainActivity : BaseActivity() {
    ...
    companion object {
        ...
        private const val BASE_URL = "https://linguagt.com"  // Fixed domain to match validation
    }
    ...
    // Expose WebView for WebAppBridge
        ...
        permissions.entries.forEach { (permission, isGranted) ->
            if (isGranted) {
                ...
                if (permission == Manifest.permission.RECORD_AUDIO && ::webView.isInitialized) {
                    // If we have a pending deep link, load it after permission grant
    ...
    private fun handleDeepLink(intent: Intent) {
        ...
        println("DEEP LINK DEBUG: Action: $action")
        println("DEEP LINK DEBUG: Data: $data")
        
        // Check if this is a VIEW action with data (typical for deep links)
        if (Intent.ACTION_VIEW == action && data != null) {
            processDeepLinkUrl(data)
        } else {
            // No deep link, load default URL
            loadDefaultUrl()
        }
    }
    ...
    private fun processDeepLinkUrl(uri: Uri) {
        ...
        println("DEEP LINK DEBUG: Scheme: $scheme, Host: $host, Path: $path, Query: $query")
        
        // Validate that this is for our domain
        if (host == "linguagt.com" || host == "www.linguagt.com") {
            // Build the full URL to load
            ...
            println("DEEP LINK DEBUG: Loading URL: $urlToLoad")
            
            if (::webView.isInitialized) {
                webView.loadUrl(urlToLoad)
            } else {
                // Store for later if WebView isn't ready yet
            ...
            // Not our domain, load default
            println("DEEP LINK DEBUG: Unrecognized host: $host, loading default")
            loadDefaultUrl()
        }
    }
    ...
    private fun setupWebViewForConversationMode() {
        ...
        // Add JavaScript bridge for web app communication
        ...
        webView.webViewClient = object : WebViewClient() {
            override fun shouldOverrideUrlLoading(view: WebView?, request: WebResourceRequest?): Boolean {
                ...
                if (uri != null) {
                    val host = uri.host
                    if (host == "linguagt.com" || host == "www.linguagt.com") {
                        // Internal navigation, allow it
                        return false
                    } else if (url.startsWith("http://") || url.startsWith("https://")) {
                        // External link - you might want to open in browser or handle differently
                        println("CONVERSATION MODE DEBUG: External URL detected: $url")
                ...
                if (request?.isForMainFrame == true) {
                    ...
                    val errorHtml = """
                        ...
                        <body>
                            ...
                            <p>Unable to load LinguaLink. Please check your internet connection.</p>
        ...
        // Don't load URL here - let handleDeepLink do it
        setContentView(webView)
        println("CONVERSATION MODE DEBUG: WebView setup completed, waiting for URL")
    }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == MICROPHONE_PERMISSION_REQUEST) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                if (::webView.isInitialized) {
                    // Reload current page or pending deep link
                    pendingDeepLinkUrl?.let { url ->
                        webView.loadUrl(url)
                        pendingDeepLinkUrl = null
                    } ?: webView.reload()
    ...
    private fun createNativeTranslationInterface() {
        ...
        retryButton.setOnClickListener {
            println("CONVERSATION MODE DEBUG: Retrying WebView setup")
            try {
                setupWebViewForConversationMode()
                handleDeepLink(intent)
            } catch (e: Exception) {
                println("CONVERSATION MODE DEBUG: Retry failed: ${e.message}")
            }
        }
        ...
        micButton1.setOnClickListener(SafeClickListener(this) {
            ...
            // Use lifecycle-aware handler for delayed operations
    ...
    private fun updateTranslationResult(text: String) {
        ...
        if (resultView != null) {
            ...
            println("CONVERSATION MODE DEBUG: Result view not found (ID: text2)")
        }
    }
    ...
    override fun onDestroy() {
        ...
        // End tracking session safely
        try {
            tracker?.endSession()
            tracker = null
        } catch (e: Exception) {
            TestRigorLogger.logError("tracker.endSession", e)
        }
